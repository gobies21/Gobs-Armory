...
public class MaliciousScytheOverlay {
    ...
    private static int previousHitCount = 0;
    private static long lastIncreaseTime = 0;
    ...
    public static void onRenderGameOverlay(RenderGuiEvent.Post event) {
        ...
        if (Config.MALICIOUS_SCYTHE_OVERLAY.get()) {
            if (entity instanceof Player player && mc.options.getCameraType().isFirstPerson()) {
                ...
                if (mainHandItem.getItem() instanceof MaliciousScytheItem) {
                    ...
                    if (hitCount > previousHitCount || currentTime - lastIncreaseTime < 3000) {
                        ...
                        if (barWidth > 0) {
                            guiGraphics.blit(FILL_BAR_LOCATION, x, middleBarStartY, 0, 0, barWidth, middleBarHeight, 20, 2);
                        }
                    }
